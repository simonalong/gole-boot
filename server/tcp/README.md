...
gole:
  server:
    tcp:
      enable: true
      port: 9000
      # 连接激活判断的时长：支持ns、us、ms、s、m、h；默认1m（1分钟）
      active-judge-duration: 1m
```
```go
import (
...
func TestTcpServer(t *testing.T) {
    // 设置自己的编码解码器
    tcp.SetCodec(func() tcp.Codec { return &XxxCodec{} })

    // 设置处理器
    tcp.Receive(func(data []byte) ([]byte, error) {
        // 收到消息
...
// 获取活跃状态；这个配置判断依赖配置：gole.server.tcp.active-judge-duration
func GetActiveStatus() bool {}
```

## 多服务
有些服务是http、grpc、tcp同时提供的，这种使用建议使用base包，这里示例如下，具体详情请看base包
```yaml
# application.yaml
gole:
  application:
...
func TestServerTcpAndHttp(t *testing.T) {
    // tcp: 设置编码解码器
    tcp.SetCodec(func() tcp.Codec { return &ServerMsgXXXCodec{} })
	
    // http: 注册服务的路由
    server.RegisterRoute(server.HmGet, "/api/data", func(c *gin.Context) (any, error) {
        return "ok", nil
    })
  ...
  server:
    tcp:
      ...
      gnet:
        ...
        multicore: false
        # NumEventLoop被设置为启动给定数量的事件循环goroutines。请注意，非负的NumEventLoop将覆盖多核。
        num-event-loop: 3
