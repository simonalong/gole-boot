...
var EtcdHooks []BaseEtcdHook

func init() {
	config.Load()

	if config.Loaded && config.GetValueBoolDefault("gole.etcd.enable", false) {
		err := config.GetValueObject("gole.etcd", &Cfg)
		if err != nil {
...
func NewEtcdClient() (*EtcdClientWrap, error) {
	...
	if Cfg.AutoSyncInterval != "" {
		t, err := time.ParseDuration(Cfg.AutoSyncInterval)
		if err != nil {
	...
	if Cfg.DialKeepAliveTime != "" {
		t, err := time.ParseDuration(Cfg.DialKeepAliveTime)
		if err != nil {
	...
	if Cfg.DialKeepAliveTimeout != "" {
		t, err := time.ParseDuration(Cfg.DialKeepAliveTimeout)
		if err != nil {
	...
	if Cfg.DialRetry == "" {
		...
		if err != nil {
			logger.Errorf("生成etcd-client失败：%v", err.Error())
			return nil, err
		}
		etcdClient = _etcdClient
	} else if Cfg.DialRetry == "always" {
		for {
			_etcdClient, err := etcdClientV3.New(etcdCfg)
			if err != nil {
				logger.Errorf("尝试连接etcd 失败：%v", err.Error())
				continue
			}
			etcdClient = _etcdClient
			break
		...
		retryTimes := util.ToInt(Cfg.DialRetry)
		for i := 0; i < retryTimes; i++ {
			_etcdClient, err := etcdClientV3.New(etcdCfg)
			if err != nil {
				logger.Errorf("尝试连接etcd 第[%v]次失败：%v", i+1, err.Error())
				continue
			}
			etcdClient = _etcdClient
			break
	...
	bean.AddBean(constants.BeanNameEtcd, &etcdClientWrap)
...
func AddEtcdHook(hook BaseEtcdHook) {
	EtcdHooks = append(EtcdHooks, hook)
	client := bean.GetBean(constants.BeanNameEtcd)
	if client == nil {
		return
	}
...
func (etcdWrap *EtcdClientWrap) AddHook(etcdHook BaseEtcdHook) {
