...
// 通用响应
type res struct {
	Code string      `json:"code"` // 200 表示正常、其他失败
...
type Registry struct {
	RegistryGroup string `json:"registryGroup"` // 注册的类型：EXECUTOR：来自注册器, ADMIN：来自界面
	RegistryKey   string `json:"registryKey"`   // 注册的执行器名
	RegistryValue string `json:"registryValue"` // 注册的执行器地址
}
